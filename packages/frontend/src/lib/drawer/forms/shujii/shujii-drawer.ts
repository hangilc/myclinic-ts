...
function leftBox(ctx: DC, box: Box){
  ...
  c.mark(ctx, "doctorName", rows[0]);
  c.mark(ctx, "clinicName", rows[1]);
  c.mark(ctx, "clinicAddress", rows[2]);
}

function rightBox(ctx: DC, box: Box) {
  const rows: Box[] = b.splitToRows(box, b.evenSplitter(2));
  c.mark(ctx, "phone", rows[0]);
  c.mark(ctx, "fax", rows[1]);
}

function detail(ctx: DC, box: Box) {
  c.mark(ctx, "detail", b.modify(box, b.inset(2, 1, 2, 0)));
}
