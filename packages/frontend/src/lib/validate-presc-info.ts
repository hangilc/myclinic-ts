...
export async function validatePrescinfoData(
  data: PrescInfoData
...
export async function validateRP剤情報(
  group: RP剤情報,
  at: string
): Promise<string | undefined> {
  ...
  let err = await confirmUsageCode(
    group.用法レコード.用法コード,
    group.用法レコード.用法名称
  );
  if (!err) {
...
export async function confirmDrugCode(
  record: 薬品レコード,
  at: string
  ...
  if (record.情報区分 === "医薬品") {
    ...
    try {
      ...
      if (m.unit !== record.単位名) {
        return `器材マスターと単位名が一致しません:${m.unit}|${record.単位名}`;
      }
    } catch {
...
export async function confirmDrugCodesOfGroups(
  groups: RP剤情報[],
  at: string,
  opt: {
...
export async function confirmUsageCode(
  code: string,
  text: string
): Promise<undefined | string> {
  if (code === "") {
