...
type Validator = (src: unknown) => unknown;

function validate<T>(init: unknown, validators: Validator[]): T {
...
function exists(src: unknown): unknown {
  if( src == null ){
    throw new 
  }
}

function validateKouhi(obj: any): KouhiInterface {
  return {
    kouhiId: validate(obj.kouhiId, [
...
// import { BaseSchema, ErrorMessage, Pipe, StringSchema, defaultArgs, pipeResult, schemaIssue } from "valibot";
...
// export function sqlDate(fieldName: string, pipe?: Pipe<string>): SqlDateSchema;
// export function sqlDate(fieldName: string, message?: ErrorMessage, pipe?: Pipe<string>): SqlDateSchema;
// export function sqlDate(fieldName: string, arg1?: ErrorMessage | Pipe<string>, arg2?: Pipe<string>): SqlDateSchema {
//   const [message, pipe] = defaultArgs(arg1, arg2);
//   return {
//     type: 'string',
//     expects: 'sqldate',
//     async: false,
//     message: `${fieldName ? fieldName + ":" : ""}：${message}`,
//     pipe,
//     _parse(input, config) {
...
//   kouhiId: v.number(),
//   futansha: v.number("負担者番号のエラー。"),
//   jukyuusha: v.number("受給者番号のエラー。"),
//   validFrom: v.string("期限開始のエラー。", [v.regex(/^((\d{4}-\d{2}-\d{2})|(0000-00-00))$/, "期限開始のエラー。")]),
//   validUpto: v.coerce(
//     v.string("期限終了のエラー。", [v.regex(/^((\d{4}-\d{2}-\d{2})|(0000-00-00))$/, "期限終了のエラー。")]),
//     coerceToUptoDate),
//   patientId: v.number(),
//   memo: v.union([v.string([v.custom(encodedJson, "メモのJSON形式エラー。")]), v.undefined_()]),
// });

// export function KouhiSchemaVariant(validFrom: BaseSchema<any, string>) {
//   return v.object({
//     kouhiId: v.number(),
//     futansha: v.number("負担者番号のエラー。"),
//     jukyuusha: v.number("受給者番号のエラー。"),
