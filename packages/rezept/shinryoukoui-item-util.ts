...
export class ChuukasanShinryou implements TekiyouItem {
  readonly isChuukasanShinryou = true;
  shikibetsu: 診療識別コードCode;
  futanKubun: 負担区分コードCode;
  shinryouList: RezeptShinryou[];
  ...
  constructor(
    ...
    this.futanKubun = futanKubun;
    this.shinryouList = shinryouList;
    this.shinryouList.sort((a, b) => {
      const aOrder = a.master.chuukasanOrder;
      if (aOrder == null) {
        throw new Error(`null chuukasan order ${a.master}`)
      }
      const bOrder = b.master.chuukasanOrder;
      if (bOrder == null) {
        throw new Error(`null chuukasan order ${b.master}`)
      }
      return aOrder.charCodeAt(0) - bOrder.charCodeAt(0);
    });
  }

  isSame(other: any): boolean {
    if (other instanceof ChuukasanShinryou) {
      return isEqualList(
        this.shinryouList,
        other.shinryouList,
        (a, b) => a.master.shinryoucode === b.master.shinryoucode
  ...
  get ten(): number {
    return this.shinryouList.reduce((sum, ele) => sum + ele.master.tensuu, 0);
  }
...
interface ShinryouGroup {
  kind: "HoukatsuKensa" | "Chuukasan" | "Simple",
  houkatsukensa?: string,
  chuukasan?: number,
}

function shinryouGroupClassifier(shinryou: RezeptShinryou): string {
  const g = shinryou.master.houkatsukensa;
  const c = shinryou.master.chuukasan;
  if (isHoukatsuGroup(g)) {
    return `H:${g}`;
  } else if (c != null && c > 0) {
    return `C:${c}`;
...
export function handleShinryouTekiyouOfVisit(visit: RezeptVisit,
  handler: (shikibetsu: 診療識別コードCode, sqldate: string, item: TekiyouItem) => void): void {
  ...
  withClassifiedBy(visit.shinryouList, s => s.shikibetsuCode, (shikibetsu, ss) => {
    withClassifiedBy(ss, s => s.futanKubun, (futanKubun, ss) => {
      withClassifiedBy(ss, shinryouGroupClassifier, (g, ss) => {
        if (g.startsWith("H")) {
          ...
          handler(shikibetsu, sqldate, new ChuukasanShinryou(
            shikibetsu, futanKubun, ss
          ));
        } else {
          ss.forEach(s => {
